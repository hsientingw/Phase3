# -*- coding: utf-8 -*-
"""
Created on Thu May 18 21:13:26 2017
...
def pivot(row, row_d, row_bin, col, col_d, col_bin, inter, inter_format, filter1, filter2):
    ...
    # print("done")
    return convert_to_json(new_data)
    
# function that the website will call
# input format:
# '0row+1row_d+2row_bin+
# 3col+4col_d+5col_bin+
# 6inter+7inter_format+
# 8filter1_col+9filter1_comparison+10filter1_values+
# 11filter2_col+12filter2_comparison+13filter2_values'
@app.route('/ajax-handler', methods=['POST'])
def handler():
    payload = request.data
    json_request = json.loads(payload)
    my_input = json_request['expression']
    ...
    if filter1[1] != "includes" and len(filter1[2]) > 1:
        return failure
    if filter2[1] != "includes" and len(filter1[2]) > 1:
        return failure
    
    # call pivot function
    pivot(row, row_d, row_bin, col, col_d, col_bin, inter, inter_format, filter1, filter2)
    
    # data = json.loads(JSON STRING)
    # return json.dumps(data), 200, ('Content-Type': 'application/json')

# testing code
# pivot("Nationality", False, 4, "Rating", True, 10, "Dribbling", "average", ("Weight", "less_than", 80), False)
# s = "hello hello"
# new_s = s.split(',')
# print(new_s)